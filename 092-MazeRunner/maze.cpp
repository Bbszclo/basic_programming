...
bool solveMazeRecursive(Grid &maze, int row, int col) {
    if (maze[row][col] == 'F') { //final case, when it hits the destination
        return true;
    }

    if (maze[row][col] == WALL || maze[row][col] == '.' || maze[row][col] == 'b') { //hits the wall or hit back
        return false;
    }

    maze[row][col] = '.'; //leave a . to mark the path

    if (row > 0 && solveMazeRecursive(maze, row - 1, col)) { //check up
        return true;
    }

    if (row < static_cast<int>(maze.size()) - 1 && solveMazeRecursive(maze, row + 1, col)) { //check down, have to integerize the size, otherwise error occurs
        return true;
    }


    if (col < static_cast<int>(maze[0].size()) - 1 && solveMazeRecursive(maze, row, col + 1)) { //check right
        return true;
    }

    if (col > 0 && solveMazeRecursive(maze, row, col - 1)) { //check left
        return true;
    }

    maze[row][col] = 'b'; //leave a b for the wrong try
    return false;
}
